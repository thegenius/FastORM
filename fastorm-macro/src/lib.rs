...
use syn::Attribute;
use syn::Field;
use syn::{parse_macro_input, LitStr, Data, Path, token, DataEnum, DataUnion, DataStruct, Fields, DeriveInput, FieldsNamed, Result, Error};
...
fn extract_selected_fields_name(fields: &FieldsNamed) -> impl Iterator<Item = proc_macro2::TokenStream> {
    let clone_fields = fields.clone();
    let data_expanded_members = clone_fields.named.into_iter().map(|field| {
        let field_name = field.ident.unwrap();
        let span = field_name.span();
...
fn build_args_add_clause(fields: &Vec<Field>, cloned: bool) -> impl Iterator<Item = proc_macro2::TokenStream> {
    let clone_fields = fields.clone();
...
pub fn impl_entity_macro(input: TokenStream) -> TokenStream {
    ...
    let primary_args_add: Vec<proc_macro2::TokenStream> = build_args_add_clause(&primary_fields, false).collect();
    let body_args_add:Vec<proc_macro2::TokenStream> = build_args_add_clause(&body_fields, false).collect();
    let body_args_add_cloned:Vec<proc_macro2::TokenStream> = build_args_add_clause(&body_fields, true).collect();
