...
pub trait GenericDaoMapper {
    ...
    async fn try_create(&self, entity: Self::E) -> Result<Self::E, Error> {
    ...
    async fn try_insert(&self, entity: Self::E) -> Result<bool, Error> {
        let table_name = entity.table_name('`');
        let field_names = entity.get_fields_string('`');
        let question_marks = entity
            .get_fields_name()
    ...
    async fn try_upsert(&self, entity: Self::E) -> Result<bool, Error> {
        let table_name = entity.table_name('`');
        let field_names = entity.get_fields_string('`');
        let question_marks = entity
            .get_fields_name()
    ...
    async fn try_update(&self, entity: Self::E) -> Result<bool, Error> {
        let table_name = entity.table_name('`');
    ...
    async fn remove(&self, primary: Self::P) -> Result<Self::E, Error> {
        todo!()
    }
    async fn try_delete(&self, primary: Self::P) -> Result<bool, Error> {
        let table_name = primary.table_name('`');
        let where_clause = primary.get_where_clause('`', "?");
    ...
    async fn try_search_paged(
        ...
        location: Self::L,
        selection: Self::S,
    ) -> Result<PagedList<Self::SE>, Error> {
        let table_name = location.table_name('`');
        let selected_fields = selection.get_sql_selection('`');
        let where_clause = location.get_where_clause('`', "?");
        let search_stmt = &format!(
    ...
    async fn try_purify(&self, location: Self::L) -> Result<usize, Error> {
        let table_name = location.table_name('`');
        let where_clause = location.get_where_clause('`', "?");
        let delete_stmt = &format!("DELETE FROM {} WHERE {}", table_name, where_clause);
        let args = location.into_any_arguments();
    ...
    async fn try_change(&self, location: Self::L, mutation: Self::M) -> Result<usize, Error> {
        let table_name = location.table_name('`');
